    ...
    const authStart = Effect.fn("McpHttpApi.authStart")(function* (ctx: { params: { name: string } }) {
      return yield* Effect.gen(function* () {
        if (!(yield* mcp.supportsOAuth(ctx.params.name))) {
          return yield* new UnsupportedOAuthError({ error: t("cli.mcp.mcp-server-does-not-support-oauth", { name: ctx.params.name }) })
        }
        return yield* mcp.startAuth(ctx.params.name)
      }).pipe(
        Effect.catchTag("MCP.NotFoundError", (error) =>
          Effect.fail(new McpServerNotFoundError({ name: error.name, message: t("cli.mcp.mcp-server-not-found", { name: error.name }) })),
        ),
      )
    })
    ...
    const authCallback = Effect.fn("McpHttpApi.authCallback")(function* (ctx: {
      ...
      return yield* mcp
        ...
        .pipe(
          Effect.catchTag("MCP.NotFoundError", (error) =>
            Effect.fail(
              new McpServerNotFoundError({ name: error.name, message: t("cli.mcp.mcp-server-not-found", { name: error.name }) }),
            ),
          ),
        )
    })
    ...
    const authRemove = Effect.fn("McpHttpApi.authRemove")(function* (ctx: { params: { name: string } }) {
      ...
      if (!(ctx.params.name in status))
        return yield* new McpServerNotFoundError({
          ...
          message: t("cli.mcp.mcp-server-not-found", { name: ctx.params.name }),
        })
