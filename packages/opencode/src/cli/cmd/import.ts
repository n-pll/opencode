...
export function formatImportFileError(file: string, error: FSUtil.Error) {
  if (error._tag === "PlatformError") {
    if (error.reason._tag === "NotFound") return t("cli.import.message.file-not-found", { file })
    if (error.reason._tag === "PermissionDenied") return t("cli.import.message.permission-denied")
    return t("cli.import.message.read-file-failed", { message: error.message })
  }

  const detail = error.cause instanceof Error ? error.cause.message : error.message
  return t("cli.import.message.invalid-json-file", { file, detail })
}
...
export const ImportCommand = effectCmd({
  command: "import <file>",
  describe: t("cli.import.describe"),
  builder: (yargs) =>
    yargs.positional("file", {
      describe: t("cli.import.positional.file"),
      type: "string",
      demandOption: true,
    }),
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    ...
    if (!slug) {
      ...
      process.stdout.write(t("cli.import.message.invalid-url", { baseUrl }))
    ...
    const tryFetch = (url: string) =>
      Effect.tryPromise({
        try: () => fetch(url, { headers }),
          ...
          new CliError({
            message: t("cli.import.error.fetch-failed", { message: e instanceof Error ? e.message : String(e) }),
          }),
      })
    ...
    if (!response.ok) {
      process.stdout.write(t("cli.import.message.fetch-failed", { statusText: response.statusText }))
      process.stdout.write(EOL)
      return
    }
    ...
    const shareData = yield* Effect.tryPromise({
      ...
      catch: () => new CliError({ message: t("cli.import.error.invalid-json") }),
    })
    const transformed = transformShareData(shareData)

    if (!transformed) {
      process.stdout.write(t("cli.import.message.share-not-found", { slug }))
      process.stdout.write(EOL)
      return
    }
  ...
  if (!exportData) {
    process.stdout.write(t("cli.import.message.read-failed"))
    process.stdout.write(EOL)
    return
  }
  ...
  process.stdout.write(t("cli.import.message.imported", { id: exportData.info.id }))
  process.stdout.write(EOL)
})
