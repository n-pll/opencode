  ...
  Effect.gen(function* () {
    ...
    const resolve = Effect.fnUntraced(function* (input?: RelativePath) {
      ...
      if (!FSUtil.contains(location.directory, absolute))
        return yield* Effect.die(new Error(t("core.filesystem.path-escapes-the-location")))
      const real = yield* fs.realPath(absolute).pipe(Effect.orDie)
      if (!FSUtil.contains(root, real)) return yield* Effect.die(new Error(t("core.filesystem.path-escapes-the-location")))
    ...
    return Service.of({
      ...
      read: Effect.fn("FileSystem.read")(function* (input) {
        ...
        if (info.type !== "File") return yield* Effect.die(new Error(t("core.filesystem.path-is-not-a-file")))
        return {
      ...
      list: Effect.fn("FileSystem.list")(function* (input = {}) {
        const target = yield* resolve(input.path)
        const info = yield* fs.stat(target.real).pipe(Effect.orDie)
        if (info.type !== "Directory") return yield* Effect.die(new Error(t("core.filesystem.path-is-not-a-directory")))
