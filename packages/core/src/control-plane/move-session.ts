  ...
  Effect.gen(function* () {
    ...
    const moveSession = Effect.fn("MoveSession.moveSession")(function* (input: Input) {
      ...
      if (moveChanges && !sourceRepository)
        return yield* new CaptureChangesError({ message: t("core.config.source_is_not_a_git_repository") })
      ...
      if (patch) {
        ...
        if (!repository) return yield* new ApplyChangesError({ message: t("core.config.destination_is_not_a_git_repository") })
        yield* git.change
      ...
      if (patch) {
        ...
        if (!repository)
          return yield* new ResetSourceChangesError({
            ...
            message: t("core.config.source_is_not_a_git_repository"),
          })
        yield* git.change
          .discard({
            repository,
