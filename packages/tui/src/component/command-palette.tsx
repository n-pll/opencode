...
export function CommandPaletteDialog() {
  ...
  const options = createMemo(() =>
    entries().map((entry) => {
      const desc = resolveCommandDescription(entry.command.name, t)
      return {
        title: typeof entry.command.title === "string" ? entry.command.title : entry.command.name,
        description: desc ?? (typeof entry.command.desc === "string" ? entry.command.desc : undefined),
    ...
    return [
      ...options()
        ...
        .map((option) => ({
          ...
          category: t("tui.command_palette.category.suggested"),
        })),
      ...options(),
    ]
  }

  return <DialogSelect ref={(value) => (ref = value)} title={t("tui.command_palette.title")} options={list()} />
}
