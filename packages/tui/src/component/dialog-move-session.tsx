...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: t("tui.dialog.move_session.loading"), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: t("tui.dialog.move_session.no_directories"), value: undefined }]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>{t("tui.dialog.move_session.deleting", { location: item.location })}</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>{t("tui.dialog.move_session.confirm_delete", { hint: deleteHint() })}</span>
        ) : suffix ? (
          <>
        ...
        category: item.root.directory === current ? t("tui.dialog.move_session.category.current") : t("tui.dialog.move_session.category.other"),
        titleWidth,
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: t("tui.dialog.move_session.confirm_delete_working_copy.title"),
          message: t("tui.dialog.move_session.confirm_delete_working_copy.message"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: t("tui.dialog.move_session.toast.failed_delete_project_copy"),
          ...
          showError() ? (
            <box paddingLeft={4} paddingRight={4}>
              <text fg={theme.error} attributes={TextAttributes.BOLD}>
                {t("tui.dialog.move_session.error")}
              </text>
            ...
            : [
                {
                  ...
                  title: t("tui.dialog.move_session.action.new"),
                  onTrigger: () => props.onSelect({ type: "new" }),
                },
                {
                  command: "dialog.move_session.delete",
                  title: t("tui.dialog.move_session.action.delete"),
                  disabled: (option) => {
                ...
                },
                {
                  ...
                  title: t("tui.dialog.move_session.action.refresh"),
