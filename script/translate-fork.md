Translate the fork CLI/TUI/core locale `$1` from the English source dictionaries. English is the read-only source of truth. Its copy is intentional and must never be modified, rewritten, or "improved."
...
These dictionaries power the headless CLI (yargs help/descriptions, prompts, spinners, log messages) and the TUI (command palette, dialogs, toasts, keybind descriptions, tips). The `core.` domain holds shared error messages rendered by both CLI and TUI error surfaces.
...
- Add every missing key with a natural, concise translation suitable for a command-line interface and terminal UI. Prefer the shortest natural phrasing that keeps the meaning; terminal text is read in narrow columns.
...
- Keys containing `{{count}}` are count phrases. Preserve `{{count}}` and translate the whole phrase naturally rather than composing translated fragments.
